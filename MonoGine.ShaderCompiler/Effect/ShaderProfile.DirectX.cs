...
internal class DirectX11ShaderProfile : ShaderProfile
{
    private static readonly Regex HlslPixelShaderRegex =
        new(@"^ps_(?<major>1|2|3|4|5)_(?<minor>0|1|)(_level_(9_1|9_2|9_3))?$", RegexOptions.Compiled);

    private static readonly Regex HlslVertexShaderRegex =
        new(@"^vs_(?<major>1|2|3|4|5)_(?<minor>0|1|)(_level_(9_1|9_2|9_3))?$", RegexOptions.Compiled);
    ...
    internal override void ValidateShaderModels(PassInfo pass)
    {
        ...
        if (!string.IsNullOrEmpty(pass.vsFunction))
        {
            ...
            if (major <= 3)
            {
                throw new Exception(string.Format(
                    "Invalid profile '{0}'. Vertex shader '{1}' must be SM 4.0 level 9.1 or higher!", pass.vsModel,
                    pass.vsFunction));
            }
        }
        ...
        if (!string.IsNullOrEmpty(pass.psFunction))
        {
            ...
            {
                throw new Exception(string.Format(
                    "Invalid profile '{0}'. Pixel shader '{1}' must be SM 4.0 level 9.1 or higher!", pass.vsModel,
                    pass.psFunction));
            }
        }
    }

    internal override ShaderData CreateShader(ShaderResult shaderResult, string shaderFunction, string shaderProfile,
        bool isVertexShader, EffectObject effect, ref string errorsAndWarnings)
    {
        ...
        var shaderData = ShaderData.CreateHLSL(bytecode, isVertexShader, effect.ConstantBuffers, effect.Shaders.Count,
            shaderInfo.SamplerStates, shaderResult.Debug);
