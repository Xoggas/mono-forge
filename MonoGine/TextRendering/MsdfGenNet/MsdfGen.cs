...
{
    public static unsafe void GenerateSDF(Bitmap<float> output, Shape shape, Projection projection, double range,
        ref GeneratorConfig config)
    {
        ...
        {
            var bitmapRefPtr = new IntPtr(&bitmapRef);
            var configPtrPtr = new IntPtr(configPtr);
            MsdfGenNative.msdfgen_generateSDF(bitmapRefPtr, shapeHandle, projectionHandle, range, configPtrPtr);
        }
    }

    public static unsafe void GeneratePseudoSDF(Bitmap<float> output, Shape shape, Projection projection, double range,
        ref GeneratorConfig config)
    {
        if (output.NumChannels != 1)
        {
    ...
    public static unsafe void GenerateMSDF(Bitmap<float> output, Shape shape, Projection projection, double range,
        ref MSDFGeneratorConfig config)
    {
    ...
    public static unsafe void GenerateMTSDF(Bitmap<float> output, Shape shape, Projection projection, double range,
        ref MSDFGeneratorConfig config)
    {
